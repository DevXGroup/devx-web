...
// Error boundary for the Three.js Canvas to prevent hydration crashes and SVG errors
...
> {
  ...
  componentDidCatch(error: Error, errorInfo: ErrorInfo) {
    console.warn('Canvas rendering error caught:', error.message)
    // Suppress detailed error logging for cleaner console
  }
...
export default function BraidedRopeAnimation({ className = "" }: BraidedRopeAnimationProps) {
  ...
  // For Safari: show only gradient fallback to prevent performance issues
  if (!clientDetected || isSafari) {
    return (
...
function BraidedRopeMesh({ screenSize }: { screenSize: { width: number; height: number } }) {
  ...
  class BraidedRopeCurve extends Curve<Vector3> {
    ...
    getPoint(t: number, optionalTarget = new Vector3()) {
      ...
      // Validate values to prevent SVG path errors
      const validX = isNaN(x * fadeMultiplier) ? 0 : x * fadeMultiplier
      const validY = isNaN(y) ? 0 : y
      const validZ = isNaN(z * fadeMultiplier) ? 0 : z * fadeMultiplier
      
      return optionalTarget.set(validX, validY, validZ)
    }
  ...
  useFrame((state, delta) => {
    ...
    // Remove continuous rotation to prevent performance issues
