...
class Noise {
  ...
  seed(seed: number) {
    ...
    for (let i = 0; i < 256; i++) {
      let v = i & 1 ? (this.p[i] || 0) ^ (seed & 255) : (this.p[i] || 0) ^ ((seed >> 8) & 255)
      this.perm[i] = this.perm[i + 256] = v
      const idx = ((v % 12) + 12) % 12
      this.gradP[i] = this.gradP[i + 256] = this.grad3[idx]!
    }
  }
  ...
  perlin2(x: number, y: number): number {
    ...
    const n01 = this.gradP[X + this.perm[(Y + 1) & 255]!]!.dot2(x, y - 1)
    const n10 = this.gradP[X + 1 + this.perm[Y]!]!.dot2(x - 1, y)
    const n11 = this.gradP[X + 1 + this.perm[(Y + 1) & 255]!]!.dot2(x - 1, y - 1)
    const u = this.fade(x)
...
const Waves: React.FC<WavesProps> = ({
  ...
  return (
    ...
    >
      <div
        className="absolute top-0 left-0 bg-[#4CD787] rounded-full w-2 h-2 pointer-events-none opacity-80 z-10"
        style={{
