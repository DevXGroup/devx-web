    ...
    <div className={`absolute inset-0 w-full h-full overflow-hidden ${className}`}>
      ...
      >
        ...
        >
          ...
          <spotLight position={[32, 22, 32]} angle={0.18} penumbra={1} intensity={2.6} castShadow color="#FFFFFF" />
          <spotLight position={[-32, 22, 32]} angle={0.18} penumbra={1} intensity={2.4} castShadow color="#E6E6FA" />
          
          {/* Fill lights for purple and gold enhancement */}
          <pointLight position={[14, 10, 22]} intensity={2.4} color="#9C27B0" distance={32} decay={2} />
          <pointLight position={[-14, -10, 22]} intensity={2.2} color="#4A148C" distance={30} decay={2} />
          <pointLight position={[0, 18, 14]} intensity={2.0} color="#FFD700" distance={26} decay={2} />
          ...
          <pointLight position={[-24, 6, -10]} intensity={2.8} color="#FFFACD" distance={42} decay={3} />
          
          {/* Additional highlights for metallic effect */}
          <pointLight position={[0, -12, 28]} intensity={1.8} color="#DDA0DD" distance={22} decay={2} />
          <pointLight position={[16, 28, 6]} intensity={1.6} color="#F0F8FF" distance={34} decay={2} />
          
          {/* Enhanced rim lighting for purple/gold edges */}
          <pointLight position={[-10, 2, -16]} intensity={2.0} color="#DA70D6" distance={38} decay={3} />
          <pointLight position={[10, 2, -16]} intensity={2.0} color="#FFF8DC" distance={38} decay={3} />
...
function DNAHelixMesh() {
  ...
  class SmoothMetallicRopeHelixCurve extends Curve<Vector3> {
    ...
    getPoint(t: number, optionalTarget = new Vector3()) {
      ...
      const fluidFlow = Math.sin(t * Math.PI * 1.0) * (0.4 + t * 0.3) + Math.cos(t * Math.PI * 0.5) * (0.2 + t * 0.2)
  ...
  useFrame((state, delta) => {
    ...
    const shadowFloatY = Math.sin(time * 0.23) * 0.32 + Math.sin(time * 0.49) * 0.10
    ...
    const unifiedFlowZ = Math.cos(time * 0.38) * 0.10 + Math.sin(time * 0.22) * 0.05
