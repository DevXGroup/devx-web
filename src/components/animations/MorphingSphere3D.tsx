...
const MorphingSphere3D = ({ isTyping = false, typingIntensity = 0 }: MorphingSphere3DProps) => {
  ...
  useEffect(() => {
    ...
    const createParticles = () => {
      ...
      for (let i = 0; i < 20; i++) { // Reduced particle count for subtlety
        newParticles.push({
          id: i,
    ...
    const animate = () => {
      ...
      for (let i = 0; i < points; i++) {
        ...
        const radiusModulationX = Math.sin(timeRef.current * 1.2 + angle * 1.5) * 6 + 
                                 Math.cos(timeRef.current * 0.8 + i * 0.6) * 4
        const radiusModulationY = Math.sin(timeRef.current * 1.0 + angle * 1.8) * 4 + 
      ...
      for (let i = 0; i < points; i++) {
        ...
        const cp1x = current.x + (next.x - controlPoints[(i - 1 + points) % points].x) * 0.2
        const cp1y = current.y + (next.y - controlPoints[(i - 1 + points) % points].y) * 0.2
      ...
      const gradient = ctx.createRadialGradient(centerX, centerY, 0, centerX, centerY, Math.max(baseRadiusX, baseRadiusY) * 2)
      if (isTyping) {
      ...
      ctx.strokeStyle = `rgba(${isTyping ? '255, 215, 0' : '76, 215, 135'}, ${glowIntensity + Math.sin(timeRef.current * 3) * 0.2})`
      ...
      setParticles(prevParticles => 
        prevParticles.map(particle => {
          ...
          const typingOpacity = isTyping ? baseOpacity * (1 + typingIntensity * 0.3) : baseOpacity * 0.7
          ctx.save()
          ...
          ctx.beginPath()
          ctx.arc(newParticle.x, newParticle.y, newParticle.size * (isTyping ? 1 + typingIntensity * 0.2 : 1), 0, Math.PI * 2)
          ctx.fill()
          ctx.restore()
      ...
      const pulseRadiusX = (Math.sin(timeRef.current * (isTyping ? 6 : 3)) + 1) * 12 + baseRadiusX * typingPulse
      const pulseRadiusY = (Math.sin(timeRef.current * (isTyping ? 6 : 3)) + 1) * 8 + baseRadiusY * typingPulse
      ctx.save()
    ...
    >
      ...
      <div className="absolute inset-0 opacity-60">
        {[...Array(4)].map((_, i) => (
          <motion.div
            key={i}
            className={`absolute w-0.5 h-0.5 rounded-full ${isTyping ? 'bg-[#FFD700]' : 'bg-[#4CD787]'}`}
            style={{
              left: `${30 + (i * 15)}%`,
      ...
      <motion.div
        className={`absolute top-1/3 left-1/3 w-2 h-2 rounded-full blur-sm ${isTyping ? 'bg-[#FFD700]' : 'bg-[#9d4edd]'}`}
        animate={{
      ...
      <motion.div
        className={`absolute top-2/3 right-1/3 w-1.5 h-1.5 rounded-full blur-sm ${isTyping ? 'bg-[#FFD700]' : 'bg-[#4CD787]'}`}
        animate={{
