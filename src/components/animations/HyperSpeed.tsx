...
class App {
  ...
  constructor(container: HTMLElement, options: HyperspeedOptions) {
    ...
    this.renderer.domElement.style.isolation = 'isolate'
  ...
  loadAssets(): Promise<void> {
    return new Promise((resolve) => {
      // Simplified asset loading - just resolve immediately
      // SMAA effects will use default assets if needed
      resolve()
    })
  }
  ...
  dispose() {
    ...
    // Proper WebGL cleanup to prevent context conflicts
    try {
      if (this.composer) {
...
const Hyperspeed: FC<HyperspeedProps> = ({ effectOptions = {}, startOnScroll = false }) => {
  ...
  useEffect(() => {
    if (!startOnScroll || shouldStart) return
  ...
  useEffect(() => {
    ...
    // Clear container to prevent flashing
    while (container.firstChild) {
      container.removeChild(container.firstChild)
    }
  ...
  }, [effectOptions, shouldStart])

  return (
    <div
      id="lights"
      data-ignore-parallax="true"
      className="w-full h-full pointer-events-none touch-none select-none cursor-default absolute inset-0"
      ref={hyperspeed}
      style={{
        pointerEvents: 'none',
        userSelect: 'none',
