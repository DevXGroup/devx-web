...
function ModernCube(props: any) {
  ...
  const handleFaceClick = (index: any) => {
    ...
    if (index !== activeFace && mesh.current) {
      const targetPosition = new Vector3(...(faces[index]?.position || [0, 0, 0]))
        .normalize()
        .multiplyScalar(5)
  ...
  useFrame((state, delta) => {
    ...
    if (activeFace !== null && !hovered) {
      const targetPosition = new Vector3(...(faces[activeFace]?.position || [0, 0, 0]))
        .normalize()
        .multiplyScalar(0.1)
  ...
  return (
    <group {...props}>
      ...
      <ParticleTrail
        ...
        color={activeFace !== null ? faces[activeFace]?.color || '#4CD787' : '#4CD787'}
      />
...
export default function ModernCube3D() {
