...
const LightBeamMaterial = ({ color, opacity = 0.07 }) => {
  ...
  useFrame((state) => {
    if (meshRef.current) {
      ...
      if (meshRef.current.material) {
        ...
        // Apply the color
        ;(meshRef.current.material as THREE.MeshTransmissionMaterial).color = currentColor
        ;(meshRef.current.material as THREE.MeshTransmissionMaterial).attenuationColor = currentColor
      }
    }
